...
def test_async_diloco_multinode_entrypoint_and_wrappers_are_main_relative():
    ...
    assert "ASYNC_ACTUAL_MULTINODE_MPI_DENSE_QUORUM=${ASYNC_ACTUAL_MULTINODE_MPI_DENSE_QUORUM:-0}" in launch_text
    assert "ASYNC_ACTUAL_MULTINODE_COMPILED_MPICH_QUORUM=${ASYNC_ACTUAL_MULTINODE_COMPILED_MPICH_QUORUM:-1}" in launch_text
    ...
    assert "--actual-multinode-compiled-mpich-quorum" in launch_text
    assert "--compiled-mpich-helper-bin" in launch_text
    assert "scripts/frontier/build_compiled_mpich_dense_helper.sh" in launch_text
...
def test_async_diloco_launch_wrappers_expose_required_env_knobs():
    ...
    for token in (
        ...
        "DEFAULT_E97_SEED_LATEST=/lustre/orion/bif148/proj-shared/emender/checkpoints/emender_E97_1.3B_20260709_084606_step_1282500/latest.pt",
        ...
        "TRAINING_TARGET=${TRAINING_TARGET:-E97_1.3B_step1282500_async_diloco_256n12h_20260709}",
        "SCALEOUT_VARIANT=${SCALEOUT_VARIANT:-E97_1.3B_step1282500_async_quorum_b4_k40_256n12h}",
        "OUTPUT_ROOT=${OUTPUT_ROOT:-",
...
def test_production_async_launcher_records_artifacts_and_real_command_branch():
    ...
    assert '--compiled-mpich-helper-bin "$ASYNC_COMPILED_MPICH_HELPER_BIN"' in launch_text
    assert '--compiled-mpich-ipc-dir "$ASYNC_COMPILED_MPICH_IPC_DIR"' in launch_text
...
def test_production_async_launcher_refuses_non_comparable_corrected_ladder_by_default():
    ...
    for token in (
        ...
        "actual_multinode_mpi_dense_quorum is the legacy mpi4py comparison path",
        "actual_multinode_compiled_mpich_quorum is not enabled",
        "256n production remains fail-closed until ASYNC_COMPILED_MPICH_64N_GATE_JSON",
