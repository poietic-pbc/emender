...
int Service::client_open(const ndp_open_v1* input, ndp_client_t* output) {
    ...
    *output = 0;
    ...
    if (client->role == NDP_ROLE_CONTROLLER) {
        controller_ = client->handle;
        // Generation state belongs to the service, not to the controller
        // connection.  A same-fence reconnect resumes the authoritative
        // freeze/result operations through fresh process-local client handles.
        if (freeze_operation_ && freeze_operation_->valid) {
            freeze_operation_->owner = client->handle;
        }
        if (result_operation_ && result_operation_->valid) {
            result_operation_->owner = client->handle;
        }
    }
...
int Service::client_close(ndp_client_t handle) {
    ...
    for (const auto& item : operations_)
        if (item.second->owner == handle && item.second != freeze_operation_
            && item.second != result_operation_) owned_ops.push_back(item.first);
...
class ServiceCore::Impl final {
 public:
    Service service;
};

ServiceCore::ServiceCore() : impl_(std::make_unique<Impl>()) {}
ServiceCore::~ServiceCore() = default;

int ServiceCore::client_open(const ndp_open_v1* value, ndp_client_t* output) {
    return guarded([&] { return impl_->service.client_open(value, output); });
}
int ServiceCore::client_poll_fd(ndp_client_t client, int* output) {
    return guarded([&] { return impl_->service.client_poll_fd(client, output); });
}
int ServiceCore::client_close(ndp_client_t client) {
    return guarded([&] { return impl_->service.client_close(client); });
}
int ServiceCore::layout_install(ndp_client_t client, const ndp_layout_v1* value) {
    return guarded([&] { return impl_->service.layout_install(client, value); });
}
int ServiceCore::buffer_register(ndp_client_t client, const ndp_buffer_v1* value,
                                 ndp_buffer_t* output) {
    return guarded([&] { return impl_->service.buffer_register(client, value, output); });
}
int ServiceCore::buffer_allocate(ndp_client_t client, const ndp_alloc_v1* value,
                                 ndp_buffer_t* output, int* output_fd) {
    return guarded([&] { return impl_->service.buffer_allocate(client, value, output, output_fd); });
}
int ServiceCore::buffer_seal(ndp_client_t client, ndp_buffer_t buffer) {
    return guarded([&] { return impl_->service.buffer_seal(client, buffer); });
}
int ServiceCore::buffer_release(ndp_client_t client, ndp_buffer_t buffer) {
    return guarded([&] { return impl_->service.buffer_release(client, buffer); });
}
int ServiceCore::submit(ndp_client_t client, const ndp_submit_v1* value,
                        ndp_op_t* output) {
    return guarded([&] { return impl_->service.submit(client, value, output); });
}
int ServiceCore::control(ndp_client_t client, const struct ndp_control_v1* value,
                         ndp_op_t* output) {
    return guarded([&] { return impl_->service.control(client, value, output); });
}
int ServiceCore::poll(ndp_client_t client, ndp_event_v1* events,
                      std::uint32_t capacity, std::uint32_t* count,
                      int timeout_ms) {
    return guarded([&] {
        return impl_->service.poll(client, events, capacity, count, timeout_ms);
    });
}
int ServiceCore::result_view(ndp_client_t client, ndp_op_t op,
                             ndp_result_v1* result, ndp_buffer_t* buffer,
                             int* output_fd) {
    return guarded([&] {
        return impl_->service.result_view(client, op, result, buffer, output_fd);
    });
}
int ServiceCore::op_release(ndp_client_t client, ndp_op_t op) {
    return guarded([&] { return impl_->service.op_release(client, op); });
}
int ServiceCore::metrics(ndp_client_t client, ndp_metrics_v1* output) {
    return guarded([&] { return impl_->service.metrics(client, output); });
}
