...
public:
    ...
    void update(const void* input, std::size_t bytes) noexcept {
        const auto* data = static_cast<const std::uint8_t*>(input);
        bit_count_ += static_cast<std::uint64_t>(bytes) * UINT64_C(8);
        while (bytes != 0) {
            const std::size_t take = (bytes < block_.size() - buffered_)
                ? bytes : block_.size() - buffered_;
    ...
    std::array<std::uint8_t, 32> finish() noexcept {
        ...
        if (buffered_ > 56) {
            std::memset(block_.data() + buffered_, 0, block_.size() - buffered_);
            transform(block_.data());
            buffered_ = 0;
        }
        std::memset(block_.data() + buffered_, 0, 56 - buffered_);
        for (std::size_t index = 0; index != 8; ++index) {
            block_[63 - index] = static_cast<std::uint8_t>(message_bits >> (index * 8));
        }
        transform(block_.data());

        std::array<std::uint8_t, 32> result{};
        for (std::size_t word = 0; word != state_.size(); ++word) {
            result[word * 4] = static_cast<std::uint8_t>(state_[word] >> 24);
            result[word * 4 + 1] = static_cast<std::uint8_t>(state_[word] >> 16);
            result[word * 4 + 2] = static_cast<std::uint8_t>(state_[word] >> 8);
            result[word * 4 + 3] = static_cast<std::uint8_t>(state_[word]);
        }
        reset();
        return result;
    }
...
private:
    ...
    void transform(const std::uint8_t* block) noexcept {
        ...
        for (std::size_t index = 0; index != words.size(); ++index) {
            ...
            const std::uint32_t temporary1 = h + s1 + choose
