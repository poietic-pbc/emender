...
export default function e97V2Tools(pi: ExtensionAPI) {
  ...
  for (const field of ["owner", "budget"] as const) {
    pi.registerTool({
      name: `lookup_${field}`,
      label: `Lookup ${field}`,
      description: `Return the ${field} field for one project record`,
      parameters: Type.Object({ project: Type.String({ minLength: 1, maxLength: 80 }) }),
      async execute(_id, { project }, _signal, _update, ctx) {
        if (!/^Project-\d{6}$/.test(project)) throw new Error("project has invalid format");
        const root = await realpath(ctx.cwd);
        const canonical = await confinedFile(root, `records/${project}.txt`);
