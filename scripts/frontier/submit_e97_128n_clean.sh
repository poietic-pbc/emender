#!/bin/bash
# Submit one immutable held 128-node clean payload, install its scheduler-owned
...
BASE=${E97_128N_CLEAN_BASE:-/lustre/orion/bif148/proj-shared/emender/frontier_runs/same-allocation-clean-128n}
...
predecessor="$CURRENT_REPO/reports/frontier/e97-same-allocation-32n-${PREDECESSOR_JOB}-verdict.json"
"$EMENDER_PYTHON" - "$predecessor" "$SOURCE_SHA" "$PREDECESSOR_PAYLOAD_DIGEST" <<'PY'
import json,sys
v=json.load(open(sys.argv[1]))
assert v.get('full_pass') is True, '32-node predecessor full_pass is not literal true'
assert v.get('source_sha') == sys.argv[2], '32-node predecessor source mismatch'
assert v.get('payload_digest') == sys.argv[3], '32-node predecessor payload mismatch'
PY
...
payload_digest=$(
  { printf '%s\n' "$config"; sha256sum \
      "$SCRIPT_DIR/e97_128n_clean_payload.sh" \
      "$SCRIPT_DIR/e97_128n_clean_collector.sh" \
      "$SCRIPT_DIR/submit_e97_128n_clean.sh" "$predecessor";
...
cp "$SCRIPT_DIR/e97_128n_clean_payload.sh" "$payload_root/payload.sh"
cp "$SCRIPT_DIR/e97_128n_clean_collector.sh" "$payload_root/collector.sh"
cp "$SCRIPT_DIR/submit_e97_128n_clean.sh" "$payload_root/submit.sh"
...
run_id="e97-128n-clean-$(date -u +%Y%m%dT%H%M%SZ)-${payload_digest:0:12}"
...
export TARGET_NODES=128 MIN_NODES=128 TASKS_PER_NODE=8 CPUS_PER_TASK=7
...
payload_id=$(sbatch --parsable --hold --no-kill --requeue --signal=B:TERM@300 \
  -A bif148 -p batch -q normal -J e97-128n-clean -N128 -t 00:45:00 \
...
import json
v={'schema':'e97-same-allocation-128n-clean-submission-v1','payload_job_id':'$payload_id',
'source_sha':'$SOURCE_SHA','payload_digest':'$payload_digest','run_id':'$run_id',
'nodes':128,'world_size':1024,'partition':'batch','qos':'normal','held':True,'released':False,
...
collector_id=$(sbatch --parsable -A bif148 -p batch -q normal -N1 -t 00:12:00 \
  --dependency="afterany:$payload_id" -J collect-e97-128n-clean \
...
[[ $job_record == *"NumNodes=128"* && $job_record == *"Partition=batch"* && $job_record == *"QOS=normal"* ]] || {
