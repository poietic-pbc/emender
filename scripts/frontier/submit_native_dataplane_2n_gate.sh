...
ARTIFACT_NAMESPACE_TOOL="$REPO/scripts/frontier/native_g2_artifact_namespace.py"
[[ -x $NDP_PYTHON_BIN ]] || {
  echo "bounded native gate Python is unavailable: $NDP_PYTHON_BIN" >&2; exit 66;
}
[[ -r $ARTIFACT_NAMESPACE_TOOL ]] || {
  echo "native G2 artifact namespace helper is unavailable" >&2; exit 66;
}
...
"$NDP_PYTHON_BIN" "$ARTIFACT_NAMESPACE_TOOL" init-root \
  --artifact-root "$NDP_ARTIFACT_ROOT"
...
[[ $JOB_ID =~ ^[1-9][0-9]*$ ]] || {
  echo "sbatch returned an invalid native G2 job identity: $SBATCH_RESULT" >&2
  exit 74
}

# The job identity is emitted before observation so an operator can always
# reconcile a successful scheduler side effect even if evidence storage fails.
printf '%s\n' "$JOB_ID"
"$NDP_PYTHON_BIN" "$ARTIFACT_NAMESPACE_TOOL" record-controller \
  --artifact-root "$NDP_ARTIFACT_ROOT" --job-id "$JOB_ID" --kind submitted \
  --evidence-json '{"scheduler":"slurm","submission":"accepted"}'
"$NDP_PYTHON_BIN" "$ARTIFACT_NAMESPACE_TOOL" observe-scheduler \
  --artifact-root "$NDP_ARTIFACT_ROOT" --job-id "$JOB_ID" --kind immediate
