...
def render(profile,out):
    c=load(); p=c["profiles"][profile]; out.mkdir(parents=True,exist_ok=True)
    ...
    lines=["#!/bin/bash",f"#SBATCH --account={s['account']}","#SBATCH --job-name=e97-async-256-@PROFILE@",f"#SBATCH --partition={p['queue']['partition']}",f"#SBATCH --qos={p['queue']['qos']}",f"#SBATCH --nodes={s['nodes']}",f"#SBATCH --time={p['walltime']}",f"#SBATCH --ntasks-per-node={s['ranks_per_node']}",f"#SBATCH --cpus-per-task={s['cpus_per_rank']}",f"#SBATCH --gpus-per-task={s['gpus_per_rank']}",f"#SBATCH --gpu-bind={s['gpu_bind']}",f"#SBATCH --export={s['export']}",f"#SBATCH --signal={s['signal']}","#SBATCH --output=e97-async-256-@PROFILE@-%j.out","#SBATCH --error=e97-async-256-@PROFILE@-%j.err","","set -euo pipefail",f"BUNDLE_FINGERPRINT='{('@FINGERPRINT@')}'",'BUNDLE_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)','REPO=$(git -C "${SLURM_SUBMIT_DIR:?}" rev-parse --show-toplevel)','test "$(sha256sum "$BUNDLE_DIR/fingerprint.sha256" | cut -d\' \' -f1)" = "$(cat "$BUNDLE_DIR/fingerprint-file.sha256")" || exit 72','RUN_ID="e97-256-${SLURM_JOB_ID:?}"','RUN_DIR="${SLURM_SUBMIT_DIR}/e97-256-runs/${SLURM_JOB_ID}"','METRICS="${RUN_DIR}/metrics.json"','HELPER="${RUN_DIR}/compiled_mpich_dense_helper"','IPC="${TMPDIR:-/tmp}/e97-256-${SLURM_JOB_ID}/ipc"','COORDINATOR_HOST=$(scontrol show hostnames "$SLURM_NODELIST" | head -1)','COORDINATOR_PORT=29497','mkdir -p "$RUN_DIR" "$IPC"','cd "$REPO"','ARTIFACT_DIR="$RUN_DIR" OUT="$HELPER" scripts/frontier/build_compiled_mpich_dense_helper.sh','exec '+" ".join(shlex.quote(x) for x in srun+["bash","-lc",payload]),""]
    ...
    normscript="\n".join(lines).replace(p["walltime"],"@WALLTIME@").replace(p["queue"]["partition"],"@PARTITION@").replace(p["queue"]["qos"],"@QOS@").replace("--walltime-remaining-s "+str(p["trainer_duration_seconds"]),"--walltime-remaining-s @DURATION@")
    ...
    (out/"rendered.sbatch").write_text((out/"rendered.sbatch").read_text().replace("@FINGERPRINT@",fp))
    return fp
def main():
