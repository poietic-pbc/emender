...
def render(profile,out):
    ...
    lines=["#!/bin/bash",f"#SBATCH --account={s['account']}","#SBATCH --job-name=e97-async-256-@PROFILE@",f"#SBATCH --partition={p['queue']['partition']}",f"#SBATCH --qos={p['queue']['qos']}",f"#SBATCH --nodes={s['nodes']}",f"#SBATCH --time={p['walltime']}",f"#SBATCH --ntasks-per-node={s['ranks_per_node']}",f"#SBATCH --cpus-per-task={s['cpus_per_rank']}",f"#SBATCH --gpus-per-task={s['gpus_per_rank']}",f"#SBATCH --gpu-bind={s['gpu_bind']}",f"#SBATCH --export={s['export']}",f"#SBATCH --signal={s['signal']}","#SBATCH --output=e97-async-256-@PROFILE@-%j.out","#SBATCH --error=e97-async-256-@PROFILE@-%j.err","","set -euo pipefail",f"BUNDLE_FINGERPRINT='{('@FINGERPRINT@')}'","BUNDLE_MANIFEST_SHA256='@BUNDLE_MANIFEST@'",f"BUNDLE_DIR={shlex.quote(str(out))}",f"REPO={shlex.quote(str(ROOT))}",f"LAUNCHER={shlex.quote(str(ROOT/c['launcher']['python']))}",'test -d "$BUNDLE_DIR" || { echo "immutable bundle is unavailable: $BUNDLE_DIR" >&2; exit 72; }','test -r "$BUNDLE_DIR/bundle-files.sha256" || { echo "bundle manifest is unavailable" >&2; exit 72; }','test "$(sha256sum "$BUNDLE_DIR/bundle-files.sha256" | cut -d\' \' -f1)" = "$BUNDLE_MANIFEST_SHA256" || { echo "bundle manifest hash mismatch" >&2; exit 72; }','if ! command -v module >/dev/null 2>&1; then source /etc/profile; fi','command -v module >/dev/null 2>&1 || { echo "module command is unavailable" >&2; exit 72; }']+module_commands+environment_commands+['command -v CC >/dev/null 2>&1 || { echo "declared modules did not provide CC" >&2; exit 72; }','test -x "$LAUNCHER" || { echo "declared launcher is unavailable: $LAUNCHER" >&2; exit 72; }','RUN_ID="e97-256-${SLURM_JOB_ID:?}"','RUN_DIR="${SLURM_SUBMIT_DIR:?}/e97-256-runs/${SLURM_JOB_ID}"','METRICS="${RUN_DIR}/metrics.json"','HELPER="${RUN_DIR}/compiled_mpich_dense_helper"','IPC="${TMPDIR:-/tmp}/e97-256-${SLURM_JOB_ID}/ipc"','COORDINATOR_HOST=$(scontrol show hostnames "${SLURM_NODELIST:?}" | head -1)','COORDINATOR_PORT=29497','export RUN_ID RUN_DIR METRICS HELPER IPC COORDINATOR_HOST COORDINATOR_PORT','mkdir -p "$RUN_DIR" "$IPC"','cd "$REPO"',preflight+' --phase before-helper','ARTIFACT_DIR="$RUN_DIR" OUT="$HELPER" scripts/frontier/build_compiled_mpich_dense_helper.sh',preflight+' --phase before-srun --helper "$HELPER"','exec '+" ".join(shlex.quote(x) for x in srun+["bash","-lc",payload]),""]
