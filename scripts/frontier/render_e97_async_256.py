...
def render(profile,out):
    ...
    module_bootstrap='if ! command -v module >/dev/null 2>&1; then test -r /opt/cray/pe/lmod/lmod/init/bash || { echo "Cray Lmod initialization is unavailable" >&2; exit 72; }; source /opt/cray/pe/lmod/lmod/init/bash; test -r /etc/profile.d/olcf-env.sh || { echo "OLCF module site initialization is unavailable" >&2; exit 72; }; source /etc/profile.d/olcf-env.sh || command -v module >/dev/null 2>&1; fi'
