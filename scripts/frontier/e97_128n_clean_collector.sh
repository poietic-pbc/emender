#!/bin/bash
#SBATCH -A bif148
#SBATCH -J collect-e97-128n-clean
#SBATCH -p batch
#SBATCH -q normal
#SBATCH -N 1
#SBATCH -t 00:12:00
...
import hashlib, json, math, os, pathlib, re, statistics, sys
import torch
...
need(re.search(rf'^{re.escape(job)}\|[^|]+\|128\|[^|]*\|batch\|normal\|',held,re.M) is not None,
     'held squeue lacks Nodes=128 Partition=batch QOS=normal')
need(re.search(rf'^{re.escape(job)}\|RUNNING\|128\|[^|]*\|batch\|normal\|',live,re.M) is not None,
     'live squeue lacks RUNNING Nodes=128 Partition=batch QOS=normal')
...
need(submission.get('predecessor_job') == '5127202', 'immediate 32-node predecessor mismatch')
need(submission.get('predecessor_payload_digest') == '6c2ca2e10120156d260befddbbc41cd2e31d975edaf4361dfa2b385ace4ee4c6',
     'immediate 32-node predecessor payload mismatch')
need(config.get('NODES')=='128' and config.get('WORLD_SIZE')=='1024', 'rendered 128-node/1024-rank config mismatch')
need(config.get('SEED_SHA256') == submission.get('seed_sha256'), 'initial/final seed identity mismatch')
need(text(run/'identity'/'payload-digest.txt').strip() == submission.get('payload_digest'), 'payload identity file mismatch')
...
if top:
    ...
    need(top[5]=='128' and top[8]=='batch' and top[9]=='normal',
         'terminal accounting lacks NNodes=128 Partition=batch QOS=normal')
train_steps=[p for p in step_rows if p[6]=='1024']
need(len(train_steps)==1 and train_steps[0][2]=='COMPLETED' and train_steps[0][3]=='0:0',
     'clean child step is not one COMPLETED 1024-task execution epoch')
...
need(len(rows)==1 and rows[0]['rc']==0 and (rows[0]['nodes'],rows[0]['tasks'])==(128,1024),
     'launcher did not retain one clean 128-node/1024-rank epoch')
...
init_ranks={int(x) for x in re.findall(r'\[(?:DiLoCo|DDP)\] rank (\d+)/1024 bound to',stdout)}
world_ranks={int(x) for x in re.findall(r'world_size=1024 backend=nccl; this is rank (\d+)',stdout)}
need(init_ranks==set(range(1024)),f'post-RCCL initialization rank set is incomplete: {len(init_ranks)}/1024')
# train.py deliberately emits the world-size announcement only from rank 0;
# the per-rank bound line is the post-init proof for all 1024 processes.
...
merge_matches=list(re.finditer(r'>>> \[DiLoCo\] merge #(\d+) at step (\d+): .*? across 1024 ranks in (\d+) ms',stdout))
...
for m in metric_re.finditer(stdout):
    try:
        metrics.append({'step':int(m.group(1)),'loss':float(m.group(2)),'global_tps':float(m.group(3)),
                        'elapsed_hours':float(m.group(4)),'time':dt.datetime.fromisoformat(m.group(5))})
...
reference_collective_ms=6529.6
collective_reference_delta_ms=collective_ms-reference_collective_ms if math.isfinite(collective_ms) else math.nan
collective_reference_ratio=collective_ms/reference_collective_ms if math.isfinite(collective_ms) else math.nan
need(math.isfinite(collective_reference_delta_ms) and math.isfinite(collective_reference_ratio),
     'direct comparison with prior proven 1024-rank 6529.6 ms reference is missing')
...
if latest.exists():
    ...
    try:
        with latest.open('rb') as f:
            for chunk in iter(lambda: f.read(64*1024*1024), b''): digest.update(chunk)
        checkpoint_sha256=digest.hexdigest()
    except OSError as exc:
        reasons.append(f'checkpoint SHA-256 failed: {exc!r}')
need(bool(re.fullmatch(r'[0-9a-f]{64}',checkpoint_sha256)), 'final checkpoint SHA-256 identity missing')
...
five_window_duration=(metric_by_step.get(checkpoint_step) or {}).get('elapsed_hours',math.nan)*3600
checkpoint_fraction_of_five_windows=(checkpoint_duration/five_window_duration
                                    if math.isfinite(checkpoint_duration) and five_window_duration>0 else math.nan)
need(math.isfinite(five_window_duration) and five_window_duration>0 and
     math.isfinite(checkpoint_fraction_of_five_windows) and checkpoint_fraction_of_five_windows>0,
     'checkpoint fraction of the first five K40 windows is missing')
...
fields={
 'schema':'e97-same-allocation-128n-clean-v2','job_id':job,
 'source_sha':submission.get('source_sha'),'payload_digest':submission.get('payload_digest'),
 'config_sha256':hashlib.sha256((run/'identity'/'config.env').read_bytes()).hexdigest(),
 'initial_seed_path':config.get('SEED_RESOLVED'),'initial_seed_sha256':config.get('SEED_SHA256'),
 'predecessor_job':submission.get('predecessor_job'),'predecessor_payload_digest':submission.get('predecessor_payload_digest'),
 'nodes_requested':128,'world_size':1024,'partition':'batch','qos':'normal',
 ...
 'prior_proven_1024_rank_collective_reference_ms':reference_collective_ms,
 'collective_reference_delta_ms':collective_reference_delta_ms,
 'collective_reference_ratio':collective_reference_ratio,
 ...
 'five_k40_window_duration_seconds':five_window_duration,
 'checkpoint_fraction_of_five_k40_windows':checkpoint_fraction_of_five_windows,
...
fields['full_pass']=not reasons and all((fields['initialized_ranks']==1024,
