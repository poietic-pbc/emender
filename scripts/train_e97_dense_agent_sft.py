...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--source-checkpoint',type=Path,required=True); p.add_argument('--source-args-json',type=Path,required=True); p.add_argument('--authority-root',type=Path,required=True); p.add_argument('--authority-sha256',required=True); p.add_argument('--pack-root',type=Path,required=True); p.add_argument('--pack-sha256',required=True); p.add_argument('--output-root',type=Path,required=True); p.add_argument('--steps',type=int,default=8); p.add_argument('--lr',type=float,default=1e-5); p.add_argument('--weight-decay',type=float,default=.01); p.add_argument('--sampler-key',type=int,default=71001); p.add_argument('--save-optimizer',action='store_true'); p.add_argument('--log-jsonl',type=Path,required=True); a=p.parse_args()
 ...
 loaded=load_e97_checkpoint(a.source_checkpoint,args_json=a.source_args_json,device=device,dtype=torch.bfloat16,weight_mode='train',use_triton=True,mmap=False); model=loaded.model.train(); model.gradient_checkpointing=True; model.gradient_checkpoint_group_size=1
 ...
 emit(a.log_jsonl,'start',source_checkpoint=str(loaded.checkpoint_path),source_step=loaded.step,world_size=world,lr=a.lr,total_parameters=sum(x.numel() for x in model.parameters()))
