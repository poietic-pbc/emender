...
def inspect_health(run_root: Path) -> dict:
    ...
    if len(worker_ranks) == 8:
        status = "torchrun active with 8 worker ranks"
    elif not torchrun and not ranks and not launch_wrappers:
        status = "stopped; no training process active"
    else:
        status = "partial or unhealthy training process set"
    return {
...
def target_eta(current: Point, target: int, rates: dict[str, dict]) -> dict:
    raw_remaining_tokens = target - current.token
    target_reached = raw_remaining_tokens <= 0
    remaining_tokens = max(0, raw_remaining_tokens)
    ...
    return {
        ...
        "tokens_over_target": max(0, -raw_remaining_tokens),
...
def main() -> None:
    ...
    gdn2_latest_step = gdn2_points[-1].step
    ...
    # GDN2 may continue beyond the completed E97 reference. Comparisons must
    # clamp to the last common logged optimizer step rather than failing or
    # extrapolating E97. The standalone GDN2 plot and status still use its true
    # latest point above.
    e97_available, _ = dedupe(e97_raw, gdn2_latest_step)
    gdn2_available, _ = dedupe(gdn2_raw, gdn2_latest_step)
    available_common = sorted(
        {p.step for p in e97_available} & {p.step for p in gdn2_available})
    if not available_common:
        raise SystemExit("no common E97/GDN2 optimizer steps")
    cutoff_step = available_common[-1]
    ...
    if common_steps[-1] != cutoff_step:
        raise SystemExit(
            f"latest common step {common_steps[-1]} does not match clamped cutoff {cutoff_step}")
    aligned = [
        {
            "step": step,
