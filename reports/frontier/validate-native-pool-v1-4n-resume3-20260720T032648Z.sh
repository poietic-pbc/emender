...
readonly RESUME_HANDOFF="$RUN_DIR/handoff/generation-00000008-fence-00000006.json"
readonly FENCE_DB="$RUN_DIR/control/pool-v1.sqlite3"
readonly BULK_ROOT=/tmp/resilient-e97-4n-20260720T032648Z
readonly KERNEL_CACHE_ROOT=/tmp/resilient-e97-4n-kernel-cache-20260720T032648Z
...
[[ $(sha256sum "$RESUME_HANDOFF" | awk '{print $1}') == 59c1bd051b1f9888283724a0c19d8c999e96f446361481450e0b0baf00293de9 ]]
[[ $(sqlite3 "$FENCE_DB" "select last_fence from lease_epochs where run_id='$RUN_ID';") == 6 ]]
[[ $(sqlite3 "$FENCE_DB" "select count(*) from leases where run_id='$RUN_ID';") == 0 ]]
[[ $(sqlite3 "$FENCE_DB" "select fence || ':' || json_extract(payload, '\$.generation') || ':' || json_extract(payload, '\$.accepted_tokens') from publications where run_id='$RUN_ID' and kind='latest' and name='authoritative';") == 6:8:57699840 ]]
