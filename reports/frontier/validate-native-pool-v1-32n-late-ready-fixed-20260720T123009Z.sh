...
readonly REPO=/lustre/orion/bif148/proj-shared/emender/source-snapshots/emender-efea17fb92
readonly SOURCE_COMMIT=efea17fbadfd5545bf9ef745c94ea56dfe8865f4
readonly SOURCE_BRANCH=wg/agent-1328/validate-native-pool-v1-32n
readonly BUILD_MANIFEST="$REPO/build/native-resilient-dataplane/native-artifacts.json"
readonly G2_GATE="$REPO/reports/frontier/native-dataplane/5035472/full-layout-gate.json"
...
readonly RESUME_HANDOFF="$RUN_DIR/handoff/generation-00000017-fence-00000020.json"
readonly FENCE_DB="$RUN_DIR/control/pool-v1.sqlite3"
readonly BULK_ROOT=/tmp/e32-f22-late-123009
readonly KERNEL_CACHE_ROOT=/tmp/e32-k-f22-late-123009
readonly NDP_SOCKET_PROBE="$BULK_ROOT/$RUN_ID/node-31/control/ndp.sock"
...
[[ $(sha256sum "$RESUME_HANDOFF" | awk '{print $1}') == 69c288476b6b6a08aaa9d3dcdc51bd748e83bdbad434d9d7c35eb3c78df9a1a5 ]]
[[ $(sqlite3 "$FENCE_DB" "select last_fence from lease_epochs where run_id='$RUN_ID';") == 21 ]]
[[ $(sqlite3 "$FENCE_DB" "select count(*) from leases where run_id='$RUN_ID';") == 0 ]]
[[ $(sqlite3 "$FENCE_DB" "select fence || ':' || json_extract(payload, '\$.generation') || ':' || json_extract(payload, '\$.accepted_tokens') from publications where run_id='$RUN_ID' and kind='latest' and name='authoritative';") == 20:17:403898880 ]]
...
exec sbatch --parsable \
  -A bif148 -p batch --qos=debug -N 32 --gpus-per-node=8 -t 00:30:00 \
  --network=job_vni \
  -J validate-native-pool-32n-late-ready-fixed --chdir="$REPO" \
  --export=ALL,REPO="$REPO",RUN_DIR="$RUN_DIR",RESILIENT_E97_RUN_ID="$RUN_ID",RESILIENT_E97_SOURCE_ID="$SOURCE_ID",RESILIENT_E97_PAYLOAD_ID="$PAYLOAD_ID",RESILIENT_E97_CODE_ID="$SOURCE_COMMIT",RESILIENT_E97_SEED="$SEED",RESILIENT_E97_TRAIN_ARGS_JSON="$REPO/configs/frontier/e97_resilient_split_role_flat.json",RESILIENT_E97_DATA="$DATA",RESILIENT_E97_TIKTOKEN_CACHE_FILE="$TIKTOKEN",RESILIENT_E97_TIKTOKEN_SHA256=94b5ca7dff4d00767bc256fdd1b27e5b17361d7b8a5f968547f9f23eb70d2069,RESILIENT_E97_NODE_COUNT=32,RESILIENT_E97_GENERATIONS=2,RESILIENT_E97_INITIAL_GENERATION=17,RESILIENT_E97_COORDINATOR_EPOCH=22,RESILIENT_E97_GLOBAL_QUORUM=31,RESILIENT_E97_GLOBAL_TOKEN_MIN=3934080,RESILIENT_E97_STARTUP_SMOKE=0,RESILIENT_E97_REQUESTED_WALLTIME=00:30:00,RESILIENT_E97_LAUNCH_MODE=node-local,RESILIENT_E97_STARTUP_DEADLINE_S=180,RESILIENT_E97_HEARTBEAT_DEADLINE_S=60,RESILIENT_E97_PROGRESS_DEADLINE_S=600,RESILIENT_E97_GENERATION_DEADLINE_S=600,RESILIENT_E97_FENCE_DB="$FENCE_DB",RESILIENT_E97_RESUME_HANDOFF="$RESUME_HANDOFF",RESILIENT_E97_BULK_ROOT="$BULK_ROOT",RESILIENT_E97_KERNEL_CACHE_ROOT="$KERNEL_CACHE_ROOT",RESILIENT_E97_MAX_SPOOL_BYTES=68719476736,RESILIENT_E97_MAX_SHARED_BYTES=68719476736,RESILIENT_E97_BULK_CHUNK_BYTES=67108864,RESILIENT_E97_LOCAL_SPOOL_CHUNK_BYTES=67108864,RESILIENT_E97_MAX_RESTARTS=2,RESILIENT_E97_INJECT_TRAINER=,RESILIENT_E97_INJECT_MANAGER=,RESILIENT_E97_INJECT_NODE_STEP=,RESILIENT_E97_INJECT_NATIVE_SERVICE=,RESILIENT_E97_DELAY_READY=31:18:120,DILOCO_DATAPLANE=native-cxi,FI_PROVIDER=cxi,FI_MR_CACHE_MONITOR=kdreg2,FI_CXI_ATS=0,NDP_BUILD_MANIFEST="$BUILD_MANIFEST",NDP_FULL_LAYOUT_GATE_JSON="$G2_GATE",EMENDER_CONDA_ENV="$APPROVED_ENV" \
