...
readonly RESUME_HANDOFF="$RUN_DIR/handoff/generation-00000016-fence-00000019.json"
readonly FENCE_DB="$RUN_DIR/control/pool-v1.sqlite3"
readonly BULK_ROOT=/tmp/e32-f20-third-114300
readonly KERNEL_CACHE_ROOT=/tmp/e32-k-f20-third-114300
...
[[ $(sha256sum "$RESUME_HANDOFF" | awk '{print $1}') == 5a8d5e921e63a72aa8e54b159e59704d30d082b7be54be61c794db174eef47bf ]]
[[ $(sqlite3 "$FENCE_DB" "select last_fence from lease_epochs where run_id='$RUN_ID';") == 19 ]]
[[ $(sqlite3 "$FENCE_DB" "select count(*) from leases where run_id='$RUN_ID';") == 0 ]]
[[ $(sqlite3 "$FENCE_DB" "select fence || ':' || json_extract(payload, '\$.generation') || ':' || json_extract(payload, '\$.accepted_tokens') from publications where run_id='$RUN_ID' and kind='latest' and name='authoritative';") == 19:16:319971840 ]]
