...
- Multi-node async entrypoint expected by the script:
  `scripts/frontier/async_diloco_e97_2n8n_debug.py`. The integration task
  restores this entrypoint from the successful 32/64-node evidence branch and
  keeps the wrapper's explicit preflight check before any approved launch.
...
the package. That task must also re-run the wrapper preflight and verify the
multi-node async entrypoint imported from the 32/64-node evidence before
submitting.
